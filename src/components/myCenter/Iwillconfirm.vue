        ...
        <article class="toConfirmContent">
            ...
            <div class="confirmBox">
                <div class="emailBox itemBox">
                    ...
                    <input type="text" name=""  v-model="email" placeholder="请输入您的邮箱地址">
                </div>
                <p class="confirmTip">请使用您所在机构的邮箱认证，163、QQ等个人邮箱不可用</p>
                <button type="button" name="button" class="submitBtn" @click='publish'>提交</button>
            </div>
        </article>
        <bounced-out v-if="showFlag" :meassage="meassage" v-on:listenToChild="showMsgChild"></bounced-out>
    </div>
</template>
<script>
            ...
            publish () {
                this.$axios({
                    ...
                    if (response.data.errno === 0) {
                        ...
                        this.meassage = response.data.message
                        this.showFlag = true
                    }
                })
            }
...
<style>
    ...
    #Iwillconfirm .toConfirmContent .confirmBox .itemBox span {
    ...
    #Iwillconfirm .toConfirmContent .confirmBox .itemBox input {
        color: #dad7d7;
        font-size: .25rem;
        border: none;
        display: block;
